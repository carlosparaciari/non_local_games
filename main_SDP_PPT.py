...
subs_Q1A2Q2 = (dimQ1,dimA2,dimQ2)
indices_Q1A2Q2 = nlg.indices_list(subs_Q1A2Q2)
...
rho_TTSS = []
shape_TTSS = (dim_TT_SS, dim_TT_SS)

for i in map(nlg.binarytoint,indices_AQ1AQ2):
    rho_TTSS.append( cp.Variable(shape_TTSS,symmetric=True) )

## OBJECTIVE FUNCTION

# The swap operator takes T(1)1 T(2)1 T(3)1 T2 S1 S2 to S1 T(2)1 T(3)1 S2 T(1)1 T2
F_TTSS = nlg.permutation_matrix((0,1,2,3), (2,3,0,1), subs_TT_SS)
...
for a1,q1,a2,q2 in indices_A1Q1A2Q2:
    ...
    obj_fun_components.append( v_a1q1a2q2 * cp.trace( cp.matmul(F_TTSS,rho_TTSS[nlg.binarytoint([a1,q1,a2,q2])]) ) )
...
trace_rho = sum([cp.trace(rho_TTSS[i]) for i in map(nlg.binarytoint,indices_AQ1AQ2)])
...
for q1,a2,q2 in indices_Q1A2Q2:
    indices_A1q1a2q2 = [nlg.binarytoint([a,q1,a2,q2]) for a in range(dimA1)]
    indices_A1Q1a2q2 = [nlg.binarytoint([a,q,a2,q2]) for a,q in indices_AQ1]
...
P = nlg.permutation_matrix((0,1,2,3),(1,0,2,3),subs_TT_SS)

for a1,q1,q2 in indices_A1Q1Q2:
    indices_a1q1A2q2 = [nlg.binarytoint([a1,q1,a,q2]) for a in range(dimA2)]
    indices_a1q1A2Q2 = [nlg.binarytoint([a1,q1,a,q]) for a,q in indices_AQ2]
...
# 5) PPT criterium
for i in map(nlg.binarytoint,indices_AQ1AQ2):
    constraints.append( nlg.partial_transpose(rho_TTSS[i],subs_TT_SS,(0,0,1,1)) >> 0 )
    constraints.append( nlg.partial_transpose(rho_TTSS[i],subs_TT_SS,(1,0,0,0)) >> 0 )
    constraints.append( nlg.partial_transpose(rho_TTSS[i],subs_TT_SS,(0,1,0,0)) >> 0 )
