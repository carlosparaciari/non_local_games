...
subs_TT1_T2_SS = (dimT,dimT,dimT,dimS,dimS)
dim_TT1_T2_SS = fc.reduce(mul, subs_TT1_T2_SS, 1)
...
rho_TTTSS = []
shape_TTTSS = (dim_TT1_T2_SS, dim_TT1_T2_SS)

for i in map(nlg.binarytoint,indices_AAQQ1_AQ2):
    rho_TTTSS.append( cp.Variable(shape_TTTSS,hermitian=True) )

## OBJECTIVE FUNCTION

# The swap operator takes T(1)1 T(2)1 T2  S1 S2 to S1 T(2)1 S2 T(1)1 T2
F_TTTSS = nlg.permutation_matrix((0,1,2,3,4), (3,1,4,0,2), subs_TT1_T2_SS)
...
for a1,q1,a2,q2 in indices_A1Q1A2Q2:
    ...
    variable_TTTSS = sum([rho_TTTSS[nlg.binarytoint([a1,aa1,q1,qq1,a2,q2])]
    					   for aa1,qq1 in indices_AQ1])
    obj_fun_components.append( v_a1q1a2q2 * cp.trace( cp.matmul(F_TTTSS,variable_TTTSS) ) )
...
trace_rho = sum([cp.trace(rho_TTTSS[i]) for i in map(nlg.binarytoint,indices_AAQQ1_AQ2)])
constraints.append( trace_rho - 1 == 0 )

# 1b) positive semidefinite matrices
for i in map(nlg.binarytoint,indices_AAQQ1_AQ2):
    constraints.append( rho_TTTSS[i] >> 0 )

# 2) Permutation invariance
# 2a) over A(1)1 Q(1)1 T(1)1 and A(2)1 Q(2)1 T(2)1

# The permutation matrix swaps T(1)1 and T(2)1
P = nlg.permutation_matrix((0,1,2,3,4), (1,0,2,3,4), subs_TT1_T2_SS)

for a1,aa1,q1,qq1,a2,q2 in indices_AAQQ1_AQ2:
    lhs = rho_TTTSS[nlg.binarytoint([a1,aa1,q1,qq1,a2,q2])]
    
    rhs_variable = rho_TTTSS[nlg.binarytoint([aa1,a1,qq1,q1,a2,q2])]
...
for aa1,q1,qq1,a2,q2 in indices_AQQ1_AQ2:
	indices_A1aa1q1qq1a2q2 = [nlg.binarytoint([a,aa1,q1,qq1,a2,q2])
									for a in range(dimA1)]
	indices_A1aa1Q1qq1a2q2 = [nlg.binarytoint([a,aa1,q,qq1,a2,q2])
									for a,q in indices_AQ1]

	lhs = sum([rho_TTTSS[i] for i in indices_A1aa1q1qq1a2q2])

	rhs_variable = sum([rho_TTTSS[i] for i in indices_A1aa1Q1qq1a2q2])
	rhs_partial = nlg.partial_trace(rhs_variable, [dimT, dim_TT_SS])
...
P = cp.Constant(nlg.permutation_matrix((0,1,2,3,4), (2,1,0,3,4), subs_TT1_T2_SS))

for a1,aa1,q1,qq1,q2 in indices_AAQQ1_Q2:
	indices_a1aa1q1qq1A2q2 = [nlg.binarytoint([a1,aa1,q1,qq1,a,q2])
									for a in range(dimA2)]
	indices_a1aa1q1qq1A2Q2 = [nlg.binarytoint([a1,aa1,q1,qq1,a,q])
									for a,q in indices_AQ2]

	lhs_variable = sum([rho_TTTSS[i] for i in indices_a1aa1q1qq1A2q2])
	lhs = cp.matmul(cp.matmul(P,lhs_variable),P)

	rhs_variable = sum([rho_TTTSS[i] for i in indices_a1aa1q1qq1A2Q2])
	rhs_permuted = cp.matmul(cp.matmul(P,rhs_variable),P)
	rhs_partial = nlg.partial_trace(rhs_permuted, [dimT, dim_TT_SS])
...
# 5) PPT criterium
for i in map(nlg.binarytoint,indices_AAQQ1_AQ2):
    constraints.append( nlg.partial_transpose(rho_TTTSS[i],subs_TT1_T2_SS,(0,0,0,1,1)) >> 0 )
#    constraints.append( nlg.partial_transpose(rho_TTTSS[i],subs_TT1_T2_SS,(1,1,0,0,0)) >> 0 )
#    constraints.append( nlg.partial_transpose(rho_TTTSS[i],subs_TT1_T2_SS,(0,0,1,0,0)) >> 0 )
...
P = cp.Constant(nlg.permutation_matrix((0,1,2,3,4), (1,0,2,3,4), subs_TT1_T2_SS))

for a1,q1,a2,q2 in indices_AQ1AQ2:
    indices_a1q1a2q2 = [nlg.binarytoint([a1,aa1,q1,qq1,a2,q2])
    					for aa1,qq1 in indices_AQ1]
    
    rho_variable = sum([rho_TTTSS[i] for i in indices_a1q1a2q2])
    rho_permuted = cp.matmul(cp.matmul(P,rho_variable),P)
    rho_TTSS = nlg.partial_trace(rho_permuted, [dimT, dim_TT_SS])
